...
static list_t *get_instrs(uint16_t start_addr) {
	...
	for (int i = 0; i < num_instrs; i++) {
		struct wsrc_instr *wsrc_instr = malloc(sizeof(*wsrc_instr));
		if (!wsrc_instr) {
			perror("get_instrs");
			return NULL;
		}
		wsrc_instr->instr = client_get_instr_at_addr(curr_addr);
		if (!wsrc_instr->instr) {
			perror("get_instrs");
			return NULL;
		}
...
static struct instruction *get_current_instr() {
	uint16_t pc = get_pc();
	return client_get_instr_at_addr(pc);
}

static void redraw_src_window(uint32_t addr) {
...
static void src_window_highlight_instr(uint32_t addr) {
	struct source_window *wsrc = &tui.src_window;
	uintptr_t uiptr_instr;
	int i = 0;
	list_for_each(wsrc->instrs, uiptr_instr) {
...
static void wsrc_move(bool up_down) {
	...
	if ((wsrc->current_pos_y == 1 && up_down == 0) ||
		...
		if (up_down == 0) {
			...
			if (first_instr->addr == 2) {
				struct instruction *instr = client_get_instr_at_addr(first_instr->addr-2);
				if (instr->len == 2)
					target_addr = 0;
				else
					target_addr = 1;
				...
				struct instruction *instr = client_get_instr_at_addr(first_instr->addr-3);
				if (instr->len == 3)
					target_addr = first_instr->addr-3;
				else {
					free(instr);
					instr = client_get_instr_at_addr(first_instr->addr-2);
					if (instr->len == 2)
			...
			redraw_src_window(target_addr);
			src_window_highlight_instr(target_addr);
		}
		else {
			struct instruction *first_instr = ((struct wsrc_instr*)wsrc->instrs->items[0])->instr;
			redraw_src_window(first_instr->addr+first_instr->len);
			src_window_highlight_instr(((struct
	...
	src_window_highlight_instr(((struct wsrc_instr
...
static void src_window_set_curr_instr(uint32_t addr) {
	struct source_window *wsrc = &tui.src_window;

	free(tui.src_window.current_instr.instr);
	wsrc->current_instr.instr = client_get_instr_at_addr(addr);

	if (!is_current_instr_in_instrs())
		redraw_src_window(wsrc->current_instr.instr->addr);
...
static void halt_and_wait() {
	...
	src_window_set_curr_instr(get_pc());
	src_window_highlight_instr(tui.src_window.current_instr.instr->addr);
	redraw_reg_window();
}
...
static void do_control_flow_next() {
	...
	src_window_set_curr_instr(get_pc());
	src_window_highlight_instr(wsrc->current_instr.instr->addr);
	redraw_reg_window();
}

static void src_window_handle_input(int input_char) {
	struct source_window *wsrc = &tui.src_window;
...
static void interpret_input(int input_char) {
	if (tui.focus_window == tui.cli_window) {
		...
		src_window_handle_input(input_char);
	}
}
